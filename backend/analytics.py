...
async def calculate_reading_stats(user_id: str, days: int, db) -> dict:
    ...
    if bookmarks_accessed_in_period > 0:
        stats["avg_reading_time_per_article"] = round(total_reading_time / bookmarks_accessed_in_period, 1)
...
async def get_reading_patterns(user_id: str, days: int, db) -> dict:
    ...
    bookmarks = await db.bookmarks.find(
        {"user_id": user_id}, {"_id": 0, "access_history": 1, "last_accessed": 1}
    ).to_list(None)
    ...
    for hour in range(24):
        for day in range(7):
            ...
            heatmap_data.append({"hour": hour, "day": day, "count": count})
