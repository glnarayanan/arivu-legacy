...
def calculate_credibility_score(
    url: str, content: str | None = None, metadata: dict | None = None
    ...
    if content:
        ...
        external_links = [link for link in links if link.lower().replace("www.", "") != domain]
    ...
    if metadata and metadata.get("author"):
        ...
        if len(author) > 2 and author.lower() not in [
            "admin",
            "administrator",
            "staff",
            "team",
...
async def check_duplicate_url(url: str, user_id: str, db) -> dict:
    ...
    existing = await db.bookmarks.find_one(
        {
            "user_id": user_id,
            "url": {
                "$regex": f"^https?://(www\\.)?{re.escape(domain)}{re.escape(path)}/?",
                "$options": "i",
            },
        },
    ...
    domain_bookmarks = await db.bookmarks.find(
        ...
        {"_id": 0, "id": 1, "title": 1, "url": 1, "created_at": 1},
    ).to_list(100)
