...
async def migrate_tracking_fields():
    ...
    try:
        ...
        missing_last_accessed = await db.bookmarks.count_documents({"last_accessed": {"$exists": False}})
        missing_view_count = await db.bookmarks.count_documents({"view_count": {"$exists": False}})
        missing_access_history = await db.bookmarks.count_documents({"access_history": {"$exists": False}})
        ...
        result2 = await db.bookmarks.update_many({"view_count": {"$exists": False}}, {"$set": {"view_count": 0}})
        ...
        result3 = await db.bookmarks.update_many(
            {"access_history": {"$exists": False}}, {"$set": {"access_history": []}}
        )
...
async def create_indexes():
    ...
    try:
        ...
        await db.bookmarks.create_index([("user_id", 1), ("last_accessed", 1)], name="user_last_accessed_idx")
        ...
        await db.bookmarks.create_index([("user_id", 1), ("view_count", -1)], name="user_view_count_idx")
...
async def create_phase2_indexes():
    ...
    try:
        ...
        try:
            await db.ai_summaries.create_index([("bookmark_id", 1)], name="bookmark_id_idx", background=True)
