        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 font-mono text-xs uppercase tracking-wider text-muted-foreground">
            <div className="flex items-center gap-1">
              {bookmark.source === 'x' && (
                <span className="inline-flex items-center justify-center w-4 h-4 bg-foreground text-background flex-shrink-0">
                  <svg className="w-2.5 h-2.5" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                  </svg>
                </span>
              )}
              {bookmark.source !== 'x' && bookmark.favicon && (
                <img src={bookmark.favicon} alt="" className="w-3 h-3" onError={(e) => e.target.style.display = 'none'} />
              )}
              <span className="truncate max-w-[120px]">
                {bookmark.source === 'x' && bookmark.x_author_username
                  ? `@${bookmark.x_author_username}`
                  : bookmark.domain}
              </span>
            </div>
            <span>•</span>
